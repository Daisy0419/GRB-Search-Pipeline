...
All times are in seconds. Replace every `None` with the corresponding measured time. Keep `np.nan` for the two original COSIpy implementations that are not run on the Jetson.
