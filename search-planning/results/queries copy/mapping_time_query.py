...
class MapTimeQueryEngine:
    ...
    def __init__(self, csv_path,
        ...
        df["n_src"] = df["n_src"] + df["n_bkg"]

        print(f"Loaded {len(df):,} rows from {csv_path}")
        print(f"  n_src       : [{df['n_src'].min():.0f}, {df['n_src'].max():.0f}]")
        print(f"  n_bkg       : [{df['n_bkg'].min():.0f}, {df['n_bkg'].max():.0f}]")
        print(f"  mapping_time: [{df['mapping_time'].min():.3f}, "
              f"{df['mapping_time'].max():.3f}] s")
        ...
        for q, k in zip(self.quantiles, self._qkeys):
            ...
            if not valid:
                print(f"  WARNING: no model converged for q={q}")
                continue
            ...
            print(f"  q={q}  best={name:30s}  R2={r['r2']:.4f}")
    ...
    def query_with_boundaries(self, n_src, n_bkg, quantile=0.99):
        """
        Same as query() but also returns the bin's (n_src, n_bkg) boundaries.

        Returns
        -------
        t          : float  predicted mapping time
        src_bounds : (src_lo, src_hi)
        bkg_bounds : (bkg_lo, bkg_hi)
        """
        t = self.query(n_src, n_bkg, quantile=quantile)

        # Resolve to bin indices
        si = int(np.clip(np.digitize(n_src, self._src_edges[1:-1]),
                        0, len(self._src_edges) - 2))
        bi = int(np.clip(np.digitize(n_bkg, self._bkg_edges[1:-1]),
                        0, len(self._bkg_edges) - 2))

        src_bounds = (self._src_edges[si],     self._src_edges[si + 1])
        bkg_bounds = (self._bkg_edges[bi],     self._bkg_edges[bi + 1])

        return t, src_bounds, bkg_bounds
