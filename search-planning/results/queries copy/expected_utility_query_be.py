...
def compute_U(t_m, b, e, D, q,
              detection_engine, map_engine,
              smoothed=True):
    """
    Single-term utility using b and e (s = e - b derived internally).
    """
    s = e - b
    if s <= 0:
        return 0.0
...
def expected_U(e, D, lambda_b, t_m, q,
    ...
    for b, w in enumerate(weights):
        ...
        s = e - b   # kept for verbose only

        u = compute_U(t_m, b, e, D, q,
                      detection_engine, map_engine,
                      smoothed=smoothed)
...
def expected_U_fast(e, D, lambda_b, t_m, q,
    ...
    while b <= e:
        ...
        if verbose:
            print(f"  b=[{b:3d},{b_end:3d}]  s=[{e-b_end:3d},{s:3d}]  "
                  f"w={float(weights[b:b_end+1].sum()):.4f}  map_t={map_t:.3f}  "
                  f"d_left={deadline_dt:.3f}  u={u:.4f}  "
                  f"contrib={float(weights[b:b_end+1].sum())*u:.4f}")
