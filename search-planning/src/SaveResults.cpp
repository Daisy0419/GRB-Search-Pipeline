...
//save result to a csv file with "sumtiles", "zenithtile", "source", "nsrc_events", and "n_bkg_events"
void save_result(const std::string& filename,
                 const std::string& method,
                 const std::string& data,
                 double budget, double slew_rate,
                 ...
                 std::vector<int> rank,
                 std::vector<int> best_path,
                 double est_time, double run_time, double padding,
                 int sum_tiles,
                 int zenithtile,
    ...
    // // Write header
    // if (!file_exists) {
    //     outfile << "Method,Dataset,Budget,SlewRate,NsrcEvents,NbkgEvents,Source,SourceTile,Detected,SumTiles,VisitedTiles,ZenithTile,SumProb,TotalCost,EstTime,RunTime,Path\n";
    // }

    // Write header
    if (!file_exists) {
        outfile << "method,dataset,budget,slew_rate,n_src,n_bkg,source,source_tile,detected,sum_tiles,visited_tiles,zenith_tile,sum_prob,detection_cost,estimate_time,runtime,path\n";
    }
    ...
    // Format source as "ra:dec" (or empty if not available)
    std::string source_str;
    if (source.size() >= 2) {
        source_str = std::to_string(source[0]) + ":" + std::to_string(source[1]);
    }

    std::string detected_str;
    if(detected) detected_str="True";
    else detected_str="False";
