...
bool readHEALPixelsCSV(const std::string& filename,
    ...
    std::cout << "Loaded " << loaded << " entries "
              << "(unique=" << seen_count
              << ", duplicates=" << dup
              << ", missing=" << missing
              << ", out-of-bound=" << oob
...
bool readHEALPixelsHDF5(const std::string& filename,
    ...
    try {
        ...
        // Read root-group attributes
        auto root = file.getGroup("/");

        if (root.hasAttribute("source")) {
            root.getAttribute("source").read(source);
        } else {
            std::cerr << "Warning: attribute 'source' not found in " << filename << "\n";
        }

        if (root.hasAttribute("n_src_events")) {
            root.getAttribute("n_src_events").read(n_src_events);
        } else {
            std::cerr << "Warning: attribute 'n_src_events' not found in " << filename << "\n";
        }

        if (root.hasAttribute("n_bkg_events")) {
            root.getAttribute("n_bkg_events").read(n_bkg_events);
        } else {
            std::cerr << "Warning: attribute 'n_bkg_events' not found in " << filename << "\n";
        }
        ...
        std::cerr << "], n_src_events = " << n_src_events
                  << ", n_bkg_events = " << n_bkg_events << "\n";
