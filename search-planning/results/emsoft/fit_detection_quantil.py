"""
Fit detection_cost ~ f(n_src, n_bkg) for one or more quantile levels.

Binning: 2D equal-width grid on (n_src, n_bkg).
"""
...
def power_src_bkg(X, a, b, c, d):
    n_src, n_bkg = X
    return a / np.power(np.maximum(n_src, 1.0), b) * np.power(np.maximum(n_bkg, 1.0), c) + d

def linear_src_bkg(X, a, b, c):
    n_src, n_bkg = X
    return a / np.maximum(n_src, 1.0) + b * n_bkg + c

def log_src_bkg(X, a, b, c, d):
    n_src, n_bkg = X
    return (a * np.log1p(n_bkg / np.maximum(n_src, 1.0)) +
            b * np.log1p(n_bkg) + c * np.log1p(n_src) + d)

def exp_ratio_bkg(X, a, b, c, d):
    n_src, n_bkg = X
    r = n_src / np.maximum(n_bkg, 1.0)
    return a * np.exp(-b * r) * np.power(np.maximum(n_bkg, 1.0), c) + d

def mm_ratio_bkg(X, a, b, c, d):
    n_src, n_bkg = X
    nb = np.maximum(n_bkg, 1.0)
    r = n_src / nb
    return a * np.power(nb, c) / (1.0 + b * r) + d

def rational_src_bkg(X, a, b, c, d, e):
    n_src, n_bkg = X
    ns = np.maximum(n_src, 1.0)
    nb = np.maximum(n_bkg, 1.0)
    return a * np.power(nb, c) / np.power(ns + b, d) + e

def stretched_exp_ratio_bkg(X, a, b, k, c, d):
    n_src, n_bkg = X
    nb = np.maximum(n_bkg, 1.0)
    r = n_src / nb
    return a * np.exp(-np.power(np.maximum(b * r, 0.0), k)) * np.power(nb, c) + d

def stretched_exp_src_times_bkg(X, a, b, k, c, d):
    n_src, n_bkg = X
    ns = np.maximum(n_src, 1.0)
    nb = np.maximum(n_bkg, 1.0)
    return a * np.exp(-np.power(np.maximum(b * ns, 0.0), k)) * np.power(nb, c) + d
...
def logistic_ratio_bkg(X, a, b, r0, k, c, d):
    ...
    return a * np.power(nb, c) * sig + d

def softplus_gap(X, a, alpha, tau, c, d):
    n_src, n_bkg = X
    nb = np.maximum(n_bkg, 1.0)
    z = np.clip((n_bkg - alpha * n_src) / np.maximum(tau, 1e-6), -60.0, 60.0)
    softplus = np.log1p(np.exp(z))
    return a * softplus * np.power(nb, c) + d
...
MODELS = {
    ...
    "log_src_bkg":                 (log_src_bkg,                 [50, 10, -10, 10],                   "a*ln(bkg/src) + b*ln(bkg) + c*ln(src) + d"),
...
def _bin_stats_2d(n_src, n_bkg, cost, n_bins_per_axis, quantiles):
    """Bin on a 2D equal-width grid of (n_src, n_bkg).

    Parameters
    ----------
    quantiles : list[float]  e.g. [0.50, 0.75, 0.90, 0.95, 0.99]
...
def fit_quantiles(df_det, output_dir=".", n_bins_per_axis=20,
                  ...
                  min_bin_count=25):
    """Fit parametric models to multiple quantile levels.
    ...
    # Colour map: one colour per quantile level
    cmap   = plt.cm.plasma
    colors = {k: cmap(i / max(len(qkeys) - 1, 1)) for i, k in enumerate(qkeys)}

    # ── Fig 1: R² comparison across quantiles ────────────────────────────────
    ...
    plt.suptitle("Model R² — Detection Cost Quantile Fits", fontsize=14)
    plt.tight_layout()
    plt.savefig(f"{output_dir}/det_quantile_r2_comparison.png",
    ...
    for i, (q, k) in enumerate(zip(quantiles, qkeys)):
        ...
        ax.set_zlabel("cost (s)", fontsize=8, labelpad=2)
        ax.set_title(f"p={q}\n{best_name}  R²={r['r2']:.4f}", fontsize=9)
        ax.tick_params(labelsize=6)
        ax.view_init(elev=25, azim=-50)

    plt.suptitle("Best-model 3D surfaces — Detection Cost Quantiles", fontsize=14, y=1.01)
    plt.tight_layout()
    plt.savefig(f"{output_dir}/det_quantile_3d_best.png", dpi=150, bbox_inches="tight")
    plt.close()
    ...
    ax.set_ylabel("Detection Cost (s)", fontsize=13)
    ax.set_title("Detection Cost — Quantile Fits Overlay", fontsize=14)
    ax.legend(fontsize=9, loc="upper right")
    ax.grid(True, alpha=0.2)
    plt.tight_layout()
    plt.savefig(f"{output_dir}/det_quantile_overlay.png", dpi=150, bbox_inches="tight")
    plt.close()

    # ── Fig 4: Pred vs target — best model, all quantiles in one figure ──────
    ncols = min(len(qkeys), 4)
    nrows = int(np.ceil(len(qkeys) / ncols))
    ...
    fig.suptitle("Pred vs Binned Target — Detection Cost Quantiles\n(dot size ∝ bin count)",
                 fontsize=13, y=1.01)
    plt.tight_layout()
    plt.savefig(f"{output_dir}/det_quantile_predvs.png", dpi=150, bbox_inches="tight")
    plt.close()
    ...
    fig.suptitle("Residuals — Detection Cost Quantile Fits", fontsize=13, y=1.01)
    plt.tight_layout()
    plt.savefig(f"{output_dir}/det_quantile_residuals.png", dpi=150, bbox_inches="tight")
    plt.close()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Fit detection cost to multiple quantile levels",
