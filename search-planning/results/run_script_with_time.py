...
#     """Index map files by (alt_int, az_int, transient_id)."""
#     index = {}
...
#         altitude = 90.0 - polar
#         key = (int(round(altitude)), int(round(azimuth)), tid)
...
# def alt_az_key(alt, az):
#     """Round to nearest 10 degrees to match the grid spacing."""
#     alt_bin = round(alt / 10) * 10
#     az_bin = round(az / 10) * 10
...
#         polar, azimuth, tid = parsed
#         altitude = 90.0 - polar
#         key = (*alt_az_key(altitude, azimuth), tid)
#         index[key] = os.path.join(map_dir, fname)
#     return index
...
def match_csv_to_maps(csv_path, map_dir):
    ...
    with open(csv_path, 'r') as f:
        ...
        for row in reader:
            ...
            # key = (int(round(alt)), int(round(az)), tid)
            # # key = (*alt_az_key(alt, az), tid)
...
if __name__ == "__main__":
    EXECUTABLE = "../build/ts"
    map_path = "/shared/valid_tests/5.36x4.5/emsoft_maps_30"
    map_stats = "/shared/valid_tests/5.36x4.5/emsoft_stats_30.csv"
    tile_path = "../data/tilings"
    source_tile_dir = "../data/source_tile3"  
