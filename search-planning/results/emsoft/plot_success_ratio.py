...
def build_binned_surface(df, value_col, n_bins, smooth_sigma, file):
    ...
    plt.tight_layout()
    plt.savefig(f"maps_distribution_{file}.png", dpi=DPI, bbox_inches="tight",
                pad_inches=0.02)
    plt.close()
...
def plot_surface_3d(SRC, BKG, prob_smooth, file, output_dir="."):
    ...
    cbar = fig.colorbar(mappable, ax=ax, shrink=0.4, pad=0.1, aspect=20)
...
def plot_all_success(df, file, output_dir=".", n_bins=20, smooth_sigma=1.0):
    SRC, BKG, prob_smooth, grouped = build_binned_surface(
        df, "detected", n_bins, smooth_sigma, file)
    ...
    for tiling in tilings:
        ...
        # for map_ in maps:
            ...
            for map_ in maps:
                ...
                file = f"{tiling}_{map_}_{deadline}"
                # print(f"Loaded {len(df)} rows from {result_csv}")
                print(f"{map_}: detected: {df['detected'].sum()} / {len(df)} "
                      f"({df['detected'].mean():.3f})")
